...
const DEFAULT_ITEMS = [
  ...
  { src: "https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=800&q=80", label: "Platillos", featured: true },
  { src: "https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=800&q=80", label: "Cortes" },
  { src: "https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=800&q=80", label: "Pastas", featured: true },
    ...
    <section id="galeria" className="py-14 sm:py-20 md:py-28 bg-cream relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mb-8 sm:mb-12">
        <div className="text-center">
          ...
          <h2 className="font-display text-2xl sm:text-4xl md:text-5xl font-bold text-chocolate leading-tight">
            Capturamos momentos
          </h2>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {items.map((img, i) => (
            <div
              key={img.label + i}
              className={`relative overflow-hidden rounded-2xl group bg-charcoal/5 shadow-md hover:shadow-xl transition-all duration-500 border border-gold/10 hover:border-gold/30 h-64 sm:h-72 md:h-[320px] ${
                img.featured ? "sm:col-span-2" : "col-span-1"
              }`}
            >
              {loaded ? (
                ...
                <div className="w-full h-full bg-chocolate/10 animate-pulse" />
              )}
              {/* Overlay with subtle label */}
              <div className="absolute inset-0 bg-gradient-to-t from-charcoal/90 via-charcoal/20 to-transparent flex items-end p-5 md:p-6 transition-opacity duration-300">
                <span className="font-display text-white text-lg sm:text-xl font-bold tracking-wide drop-shadow-md group-hover:text-gold transition-colors duration-300">
                  {img.label}
                </span>
              </div>
            </div>
