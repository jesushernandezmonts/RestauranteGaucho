...
import { Loader2, ChevronLeft, Package, Plus, X, AlertTriangle, History } from "lucide-react";
        ...
        {tab === "inventario" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {ingredientes.map((ing) => {
              ...
              return (
                <div key={ing.id} className={`card ${isLow ? "border-danger/20" : ""}`}>
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <button onClick={() => setEditIng(ing)} className="p-1 rounded-lg hover:bg-surface-light opacity-0 group-hover:opacity-100 transition-all">
                      <Package size={14} className="text-text-muted" />
                    </button>
                  </div>
                  <div className="flex items-end justify-between">
                    <div>
                      <div className={`text-2xl font-bold ${isLow ? "text-danger" : "text-text-primary"}`}>
                        {ing.stock}
                      </div>
                      <div className="text-xs text-text-muted">
                        Mín: {ing.stockMinimo} {ing.unidad}
                      </div>
                    </div>
