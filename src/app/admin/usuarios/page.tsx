          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-primary/10">
                <th className="text-left px-6 py-4 text-xs font-medium text-text-muted uppercase">Nombre</th>
                <th className="text-left px-6 py-4 text-xs font-medium text-text-muted uppercase">Usuario</th>
                <th className="text-left px-6 py-4 text-xs font-medium text-text-muted uppercase">Rol</th>
                <th className="text-center px-6 py-4 text-xs font-medium text-text-muted uppercase">Activo</th>
                <th className="text-right px-6 py-4 text-xs font-medium text-text-muted uppercase">Acción</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-primary/5">
              {usuarios.map((u) => (
                <tr key={u.id} className="hover:bg-surface-light/30">
                  <td className="px-6 py-4 text-sm font-medium text-text-primary">{u.nombre}</td>
                  <td className="px-6 py-4 text-sm text-text-secondary">{u.usuario}</td>
                  <td className="px-6 py-4 text-sm text-text-secondary">{roleLabels[u.role] || u.role}</td>
                  <td className="px-6 py-4 text-center">
                    <span className={`inline-block w-2 h-2 rounded-full ${u.activo ? "bg-success" : "bg-text-muted"}`} />
                  </td>
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => setEditUser(u)} className="p-1.5 rounded-lg hover:bg-surface-light">
